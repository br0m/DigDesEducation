...
public interface TaskRepositoryJpa extends JpaRepository<Task, Integer> {
    ...
    @Query("select t from Task t where (t.title = :title OR :title IS NULL) and (t.status = :status OR :status IS NULL) and (t.author.id = :authorId OR :authorId IS NULL) and (t.responsibleMember.id = :responsibleMemberId OR :responsibleMemberId IS NULL) and t.deadline >= :deadline and t.creationDate >= :creationDate ORDER BY t.creationDate DESC")
